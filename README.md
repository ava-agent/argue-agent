...
- **实时语音监听** - 浏览器内置语音识别，实时转录对方发言（免费，无需 API Key）
- **上下文连续分析** - 滑动窗口累积对话上下文，跨语句识别论点
- **论点提取** - 自动识别对方发言中的可验证论点
- **事实核查** - 通过 Tavily/DuckDuckGo 搜索验证论点真实性
- **反驳建议** - 生成针对性的反驳或确认建议
- **渐进式渲染** - 先展示骨架卡片，逐条加载判定结果
...
- **System Prompt 定义 JSON Schema** — 通过详细的系统提示词规定输出的 JSON 结构，并在服务端使用容错解析处理纯 JSON 或代码块包裹的 JSON
...
vercel env add ARK_BASE_URL production
vercel env add ARK_CHAT_MODEL production
...
| 论点提取 | ~1-2s | Ark JSON 结构化输出 |
| 证据搜索 | ~1-2s | DuckDuckGo 并发查询 |
| 判定综合 | ~1-2s | Ark 综合分析 |

## 配置项

LLM 配置使用共享 `ARK_*` 变量，项目专属配置继续使用 `ARGUE_` 前缀：
